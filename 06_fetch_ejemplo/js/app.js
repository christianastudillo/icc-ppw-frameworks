...
function renderizar(lista) {
  ...
  lista.forEach(item => {
    ...
    const fraces = document.createElement('p');
    if (item.phrases.length === 0) {
        fraces.textContent = 'No hay frases disponibles';
    } else {
        for (let i = 0; i < Math.min(item.phrases.length, 3); i++) {
        const p = document.createElement('p');
        p.textContent = item.phrases[i];
        fraces.appendChild(p);
      }
    }
    ...
    bloqueTexto.appendChild(fraces);
